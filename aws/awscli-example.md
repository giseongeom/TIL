...
* custom AMI의 `ImageID`, `CreationDate` 정보를 출력 그리고, sort 명령으로 `CreationDate` 기준 정렬
```bash
$ aws ec2 describe-images --query 'Images[].[ImageId,CreationDate]'  --owners self --output text | sort -k 2
...
```
* customAMI의 Tags 정보도 포함해서 출력 이 때 `Tags[?Key=='OS_Version'].Value | [0]` 구문에 유의
```bash
$ aws ec2 describe-images --query "Images[].[ImageId,CreationDate,Tags[?Key=='OS_Version'].Value | [0], Tags[?Key=='Env'].Value | [0] ]" --owners self --output text
...
```bash
$ aws ec2 describe-images --query "sort_by(Images, &CreationDate)[*].[ImageId,CreationDate]" --owners self --output text
