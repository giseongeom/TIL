...
* box 원본으로 사용할 Hyper-V VM을 셧다운 --> export
* export 받은 경로에서 *Snapshots* 폴더는 삭제함
* metadata.json 파일을 추가
파일 내용
```json
{ "provider": "hyperv" }
```

* bsdtar.exe 사용해서 .box 생성
```
c:\HashiCorp\Vagrant\embedded\gnuwin32\bin\bsdtar.exe -zcvf c:\vmstore\centos6_64.box ./*
```

* .box 파일 크기를 최소화하기 위해 lzma 알고리즘을 사용. 대신 시간이 많이 걸림
```
c:\HashiCorp\Vagrant\embedded\gnuwin32\bin\bsdtar.exe --lzma -cvf c:\vmstore\centos6_64.box ./*
```

### VMware Provider

* box 원본으로 사용할 VMware VM을 셧다운
* 임의의 폴더에 다음의 파일들을 복사함
```
.nvram
.vmdk
.vmsd
...
```
* bsdtar.exe 사용해서 .box 생성 (Hyper-V와 동일함)

### VirtualBox Provider

* box 원본으로 사용할 VM 셧다운
* `.ovf` 포맷으로 export
* 반드시 `box.ovf` 로 파일명 사용한다. 다른 .ovf 파일명을 사용하는 경우, box가 추가되지만 실제 vagrant up 과정에서 실패.
* `base MAC address` 지정된 Vagrantfile 필요
```
C:\Vagrant\Project\ubuntu>vagrant up
...
Vagrant::Config.run do |config|
  ...
  config.vm.base_mac = "0800279DDCB6"
